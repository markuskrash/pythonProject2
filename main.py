...
text_for_legend = [["Altar of the living",
                    "",
                    "",
                    "",
                    "",
                    "Щелкните мышью для продолжения"],
                   ["956 год, Скандинавия",
                    "",
                    "Это мир, где люди сосуществуют с магией. ",
                    "Но на земле существует и монстры, несущие вред людям.",
                    "Они часто нападают на селения людей, забирая много жизней",
                    "Такое событие не обошла и нашего главного героя(Эша).",
                    "Монстры ворвались в деревню,в которой он жил.",
                    "Его брат пытался защитить Эша от монстров, что у него вышло, но к сожалению"
                    " ценой своей жизни.",
                    "Наш главный герой, не смерившись, с такой утратой, решил узнать есть ли способ"
                    " воскресить брата.",
                    "Он узнал от одной ведьмы, что существуют три алтаря, найдя которые можно воскресить любого.",
                    "Но путь до него очень опасен и полон монстров.",
                    "Эш, смастеря доспехи из мусора и взяв кухонный нож, отправился к вратам"
                    "Щелкните мышью для продолжения"],
                   ["Управление:",
                    "",
                    "A - налево, D - вправо, SPACE - прыжок",
                    "Левая кнопка мыши - удар",
                    "Вы автоматически начнете игру с первого уровня",
                    "Всего в игре их три"
                    "Для перехода на следующий уровень, требуется дойти до прохода",
                    "В правой части уровня",
                    "Если вы умрете игра закроется, и проходить ее придется заново",
                    "Щелкните мышью для продолжения"]]
...
text_for_win = [["Вы прошли первый уровень!",
                 "Он был обучающим, дальше будет сложнее, удачи",
                 "Вы научились, как сражаться с разными видами монстров и убивать их",
                 "Также вы стали лучше прыгать и бегать по сравнению с началом путешествия",
                 '',
                 '',
                 'Вы заработали:',
                 '',
                 ...
                 '',
                 '',
                 "Щелкните мышью, чтобы выйти из игры"],
                ["Вы прошли второй уровень!",
                 '',
                 '',
                 'Вы заработали:',
                 '',
                 ...
                 "Щелкните мышью, чтобы выйти из игры"],
                ["Вы прошли игру!",
                 "Вы воскресили своего брата и теперь вы его защищаете, а не он вас,",
                 "как было в начале вашей скромной жизни",
                 "Вам дали второй шанс, не потеряйте брата снова, берегите его",
                 '',
                 '',
                 'Вы заработали:',
                 '',
...
class enemy_HAGIVAGI(pygame.sprite.Sprite):
    ...
    def collision_with_borders(self):
        ...
        if pygame.sprite.spritecollideany(self, sprites_of_hero):
            hero = pygame.sprite.spritecollide(self, sprites_of_hero, False)[0]
            if self.coll % 300 <= 180 and hero.is_hit <= 20 and (hero.rect.centerx - self.rect.centerx) * hero.direction_x < 0:
                self.kill()
            if self.coll % 300 > 180:
                is_end[0] = 1
...
class level:
    def __init__(self, screen, n):
        for row in range(len(levels[n])):
            for i in range(len(levels[n][row])):
                if levels[n][row][i] == '-' and (row != 0 and levels[n][row - 1][i] != '-'):
                    border(row, i, 0)
                elif levels[n][row][i] == '-' and (i != 0 and levels[n][row][i - 1] != '-'):
                    border(row, i, 2)
                elif levels[n][row][i] == '-' and (i != len(levels[n][row]) - 1 and levels[n][row][i + 1] != '-'):
                    border(row, i, 3)
                elif levels[n][row][i] == '-':
                    border(row, i, 1)
                if levels[n][row][i] == '*':
